...
const formSchema = z.object({
  ...
  questions: z.array(testQuestionSchema).min(1, 'آزمون باید حداقل یک سوال داشته باشد.'),
});
...
export function TestForm({ test, onSuccess }: TestFormProps) {
  ...
  async function onSubmit(data: FormValues) {
    ...
    try {
        if (isEditMode && test) {
            const testRef = doc(firestore, 'teachers', user.uid, 'tests', test.id);
            updateDocumentNonBlocking(testRef, { ...testData, createdAt: test.createdAt });
            toast({ title: 'آزمون به‌روزرسانی شد', description: 'تغییرات با موفقیت ذخیره شد.' });
        } else {
            const testsCol = collection(firestore, 'teachers', user.uid, 'tests');
            const newDocRef = doc(testsCol);
            setDocumentNonBlocking(newDocRef, { ...testData, id: newDocRef.id }, {});
